...
def test_all_features():
    ...
    for color in ['orange', 'green', 'blue']:
        ...
        assert state['factions'][color]['credits'] == 0, f"{color} should start with 0 credits"
    ...
    # Check that credits were not reset to a fixed value (no guaranteed weekly income)
    for color, data in state['factions'].items():
        # After weekly reset, credits continue from previous week plus daily production
    ...
    for color, data in reset_state['factions'].items():
        assert data['credits'] == 0, f"{color} credits should reset to 0"
